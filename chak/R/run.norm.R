...
cat("\n\nRunning additional simulations for normal dist.\n")
...
for (i in seq_along(sd)) {
  for (j in seq_along(delta)) {
    m10 <- list(m = 10, delta = delta[j], sd = sd[i], dist_fun = rnorm)
    m20 <- list(m = 20, delta = delta[j], sd = sd[i], dist_fun = rnorm)
    scenarios[[glue::glue("norm10.sd{i}.d{j}")]] <- m10
    scenarios[[glue::glue("norm20.sd{i}.d{j}")]] <- m20
  }
}
...
write.csv(sim_results_df, "chak/data/locshift.norm.n.csv", row.names = FALSE)
saveRDS(sim_results, "chak/data/locshift.norm.n.estimates.RDS")
...
write.csv(df, "chak/data/locshift.norm.pwr.csv", row.names = FALSE)
